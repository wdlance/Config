// mode3_medzt.js -- Mode 3: Network font interception for Medzt (macorner.co)
// Loaded in popup.html (script tag)
// scanMedztFonts is injected into page via chrome.scripting.executeScript

function scanMedztFonts(urlPrefix, initiatorTypes, pathFilter, extensionFilter) {
  console.log('[FontCapture Mode3] Scanning network fonts, prefix:', urlPrefix,
  ...
  try {
    ...
    console.log('[FontCapture Mode3] Resource entries count:', entries.length);
    for (const entry of entries) {
      ...
      console.log('[FontCapture Mode3] Matched URL:', entry.name,
        'initiatorType:', entry.initiatorType,
        'transferSize:', entry.transferSize);
      fontUrls.push(entry.name);
    }
  } catch (e) {
    console.error('[FontCapture Mode3] Performance API exception:', e.message, e.stack);
  }
  console.log('[FontCapture Mode3] All prefix-matched:', allMatched.length,
    'Filtered out:', filteredOut.length);
  filteredOut.forEach(f => console.log('[FontCapture Mode3]   Filtered:', f.url, f.reason));
  const unique = [...new Set(fontUrls)];
  console.log('[FontCapture Mode3] Final font URLs (deduplicated):', unique.length);
  unique.forEach(u => console.log('[FontCapture Mode3]   URL:', u));
